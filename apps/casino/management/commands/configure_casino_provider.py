...
class Command(BaseCommand):
    help = "Create or update a casino provider with base_url and config (api_key, secret, etc.)"
    ...
    def handle(self, *args, **options):
        ...
        provider, _ = CasinoProvider.objects.get_or_create(key=key, defaults={
            'name': name,
            'base_url': base_url,
            'active': active,
        })
        ...
        # Merge credentials into config JSON
        ...
        self.stdout.write(self.style.SUCCESS(
            f"Provider '{provider.key}' configured. Active={provider.active} BaseURL='{provider.base_url}'"
        ))
